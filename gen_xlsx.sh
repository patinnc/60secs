...
echo "SCR_DIR= $SCR_DIR" >&2

export LC_ALL=C
export PATH=$SCR_DIR/:$PATH
CK_GAWK=$(which gawk)
if [ "$CK_GAWK" == "" ]; then
  echo "$0.$LINENO didn't find gawk in path. bye"
  echo "$0.$LINENO one solution is to:"
  echo "cp ~/patrick_fay_bin/gawk ."
  echo "cp ~/patrick_fay_bin/gawk awk"
  CDIR=$(pwd)
  echo "export PATH=$CDIR:\$PATH"
  echo "export LC_ALL=C"
  exit 1
fi
CK_GAWK=$(awk --version | grep "GNU Awk")
if [ "$CK_GAWK" == "" ]; then
  echo "$0.$LINENO scripts require awk to be mapped to gawk. bye"
  echo "$0.$LINENO one solution is to:"
  echo "cp ~/patrick_fay_bin/gawk ."
  echo "cp ~/patrick_fay_bin/gawk awk"
  CDIR=$(pwd)
...
if [ "$NUM_DIR_IN" != "" ]; then
  ...
  IFS=$IFS_SV
  echo "NUM_DIR_ARR= ${NUM_DIR_ARR[@]}, BM0= ${NUM_DIR_ARR[0]}, BM1=${NUM_DIR_ARR[1]}" >&2
  ...
  echo "NUM_DIR_ARR= ${NUM_DIR_ARR[@]}, ND0= ${NUM_DIR_ARR[0]}, ND1=${NUM_DIR_ARR[1]} NUM_DIR_BEG= $NUM_DIR_BEG NUM_DIR_END= $NUM_DIR_END" >&2
fi

ck_skip_dir_due_to_num_dir () {
  #echo "$0.$LINENO dir_num= $1 NUM_DIR_IN= $NUM_DIR_IN ck beg $NUM_DIR_BEG end= $NUM_DIR_END" >&2
  if [ "$1" != "" ]; then
    local j=$1
      if [ "$NUM_DIR_BEG" != "" ]; then
         if [ "$j" -lt "$NUM_DIR_BEG" ]; then
         echo "$0.$LINENO  job_id= $JOB_ID skip dir $j due to -N $NUM_DIR_IN option called by line $2" >&2
         return 1
         fi
      fi
      if [ "$NUM_DIR_END" != "" ]; then
...
get_dir_list() {
   ...
   DIR=$STR
   echo "$0.$LINENO +___________-- get_dir_list: j= $j DIR= $DIR" >&2
}

OPT_a=
if [ "$AVG_DIR" != "" ]; then
  if [ "$AVERAGE" == "0" ]; then
     echo "$0: cmdline options has -a $AVG_DIR but you didn't specify -A option. Bye" >&2
     exit 1
  fi
  if [ -d $AVG_DIR ]; then
    mkdir -p "$AVG_DIR"
...
if [ "$DESC_FILE" != "" ]; then
   if [ $VERBOSE -gt 0 ]; then
     echo "$0.$LINENO DESC_FILE= $DESC_FILE" >&2
   fi
  ...
  if [ -e $INPUT_FILE_LIST ]; then
   ...
   if [ $RESP -eq 0 ]; then
     ...
     if [ "$GOT_DIR" == "0" ]; then
       ...
       RESP=`find $DIR -name "$CKF" | wc -l | awk '{$1=$1;print}'`
       if [ $VERBOSE -gt 0 ]; then
         echo "$0.$LINENO got29 $RESP $CKF file(s) under dir $DIR. Using the dir of first one if more than one." >&2
       fi
     if [ "$RESP" == "0" ]; then
       CKF="run_itp.log"
       RESP=`find $DIR -name $CKF | wc -l | awk '{$1=$1;print}'`
     ...
     if [ "$RESP" == "0" ]; then
       ...
       if [ "$RESP" == "0" ]; then
         ...
         RESP=`find $DIR -name "$CKF" | wc -l | awk '{$1=$1;print}'`
         if [ $VERBOSE -gt 0 ]; then
           echo "$0.$LINENO got $RESP $CKF file(s) under dir $DIR. Using the dir of first one if more than one." >&2
         fi
       fi
       if [ "$RESP" != "0" ]; then
         if [ $VERBOSE -gt 0 ]; then
           echo "$0.$LINENO found $RESP $CKF file(s) under dir $DIR. Using the dir of first one if more than one." >&2
         fi
  ...
  /^# started on / {
    ...
    printf("%s\n", tst_epoch);
      ...
      ck_last_rc $? $LINENO
             #TS_INIT=
            if [ $VERBOSE -gt 0 ]; then
             echo "$0.$LINENO first sys_*_perf_stat.txt TS_INIT= $TS_INIT" >&2
      ...
      STR="$STR $NM"
         ...
         DIR_OUT="$DIR_OUT $STR"
         if [ $VERBOSE -gt 0 ]; then
         echo "$0.$LINENO: -__________- using2 DIR= $DIR, orig DIR= $DIR_ORIG" >&2
         fi
       else
...
#xyz
      ...
      LST=$DIR
      if [ $VERBOSE -gt 1 ]; then
        echo "+__________$0.$LINENO input dirs= $itot. got matches= $ii for skus= ${SKU[@]}" >&2
      fi
   fi
fi
#echo "$0.$LINENO bye"
...
if [ $VERBOSE -gt 0 ]; then
  echo "$0.$LINENO LST= $LST" >&2
fi

#echo "$0.$LINENO: DIR_LST= $LST" >&2
DIR_1ST_DIR=
if [ "$INPUT_FILE_LIST" == "" ]; then
  NUM_DIRS=0
  for i in $LST; do
...
for i in $LST; do
 ...
 if [ "$BEG_TM_IN" != "" ]; then
  ...
  echo "$0.$LINENO ____got BEG_TM_IN=\"$BEG_TM_IN\"" >&2
 fi
 OPT_END_TM=
 ...
 if [ -e $CDIR/job_${JOB_ID}.stop ]; then
    ...
    echo "$0: got job_$JOB_ID.stop pid= $RESP and bashpid= $$" >&2
    if [ "$RESP" == "$$" ]; then
 ...
 if [ "$SKIP_SYS_2_TSV" == "0" ]; then
   ...
   if [ "$BACKGROUND" -le "0" ]; then
     ...
     #echo "$0.$LINENO job_id= $JOB_ID jbs= $jbs LOAD= $LOAD BACKGROUND= $BACKGROUND" >&2
     jbs=$(($jbs+$LOAD))
   ...
   if [ "$jbs" -gt "$BACKGROUND" ]; then
     ...
     do
       ...
       RC=$?
       if [ "$RC" != "0" ]; then
          echo "$0: sys_2_tsv.sh got error! bye at $LINENO" >&2
          echo "$0: look at ${BK_OUT[$LPID]} file in last data dir for error messages" >&2
 ...
 TS_DFF=$(($TS_CUR-$TS_BEG))
 if [ $VERBOSE -gt 0 ]; then
   echo "$0.$LINENO FLS: job_id= $JOB_ID dir_num= ${DIR_NUM} of ${DIR_NUM_MX}, elap_tm= $TS_DFF secs, ${FLS}" >&2
 fi
 DIR_NUM=$(($DIR_NUM+1))
 if [ $VERBOSE -gt 0 ]; then
   popd
...
wait_for_all() {
     ...
     do
       ...
       #if [ "$RC" == "1" -o "$RC" == "2" ]; then
       if [ "$RC" != "0" ]; then
          echo "$0:$LINENO sys_2_tsv.sh got error RC= \"$RC\"! at $LINENO. bye. called by line $1" >&2
          echo "$0:$LINENO look at ${BK_OUT[$job]} in last data dir for error messages" >&2
...
# fi
 ...
 for ((k=i_idx; k < i_idx+1; k++)); do
   ...
   RESP=${SHEETS_OUT[$k]}
   if [ $VERBOSE -gt 0 ]; then
     echo "$0.$LINENO ckdir[$k] dir= $i resp= $RESP" >&2
   fi
   if [ "$RESP" != "" ]; then
     if [ $VERBOSE -gt 0 ]; then
     ...
     for ((kk=0; kk < ${#SHEET_FILES[@]}; kk++)); do
       ...
       if [[ $flnm == *"perf_stat"* ]]; then
         FLS_PS=$SDIR/$flnm
         #echo "$0.$LINENO perf_stat file= $FLS_PS" >&2
       fi
       if [[ $flnm == *"mpstat"* ]]; then
         FLS_MP=$SDIR/$flnm
       fi
     ...
     if [ "$missed_files" != "" ]; then
       echo "$0.$LINENO: !!!!!!!!!!!!!!! SHEETS missed_files= $missed_files" >&2
     fi
     break
   fi
 done
...
#   echo "$0.$LINENO ++++++++FLS_PS= $FLS_PS" >&2
#   if [ "$FLS_PS" == "" ]; then
#     FLS_PS=`ls -1  $i/../*txt.tsv | grep perf_stat`
#     echo "$0.$LINENO ++++++++FLS_PS= $FLS_PS"  >&2
#     if [ "$FLS_PS" != "" ]; then
#       echo "$0.$LINENO ++++++++FLS= $FLS" >&2
...
#abc
  ...
  echo "$0.$LINENO got here"
    ...
    echo "$0.$LINENO $SCR_DIR/redo_chart_table.sh -O "$OPTIONS" -S $SUM_ALL -f $ALST -o $OFILE   -g perf_stat $OPT_METRIC -r 50 -t __all__" >&2
  ...
  if (allow_zero0_nonzero1 == 0 || (arr[fld_v] != "" && arr[fld_v] != 0)) {
    ...
    { if (index($0, sum_file) > 0 || index($0, sum_all) > 0) {
        ...
        while ((getline line < flnm) > 0) {
           ...
           if (mtrcm1 == "mutt_RPS_host_calls_val_arr" && index(mtrc, " val_arr") > 0) {
              pre_do_pxx_compare(mtrcm1, mtrc, fld_v, arr, fls, fld_beg, n, 1);
           }
...
#abc
  ...
  echo "$0.$LINENO ____got BEG_TM_IN=\"$BEG_TM_IN\""  >&2
    ...
    USE_DIR=
    if [ $VERBOSE -gt 0 ]; then
      echo "$0.$LINENO find $DIR_1ST_DIR -name 60secs.log"  >&2
    fi
  ...
  echo "$0.$LINENO ____got BEG_TM_IN=\"$BEG_TM_IN\""  >&2
      ...
      if [ $VERBOSE -gt 0 ]; then
        echo "$0.$LINENO beg_tm= $BEG_TM end_tm= $END_TM"  >&2
        echo "$0.$LINENO $BEG_TM" | awk '{print strftime("beg_time: %c %Z",$1)}'  >&2
        echo "$0.$LINENO $END_TM" | awk '{print strftime("end_time: %c %Z",$1)}'  >&2
      fi
      ...
      if [ "$RESP_ITP" != "0" ]; then
         ...
         ITP_INTRVL=`awk '
          ...
          ' $ITP_LOG`
         if [ $VERBOSE -gt 0 ]; then
           echo "$0.$LINENO ITP_INTERVAL= $ITP_INTRVL, log= $ITP_LOG"  >&2
         fi
      fi
  ...
  echo "$0.$LINENO got to here________________"  >&2
      echo -e "-p\t\"$RPS\"" >> $ALST
      echo -e "-s\t$CHART_SIZE" >> $ALST
      if [ "$DESC_FILE" != "" ]; then
        echo -e "-d\t\"$DESC_FILE\"" >> $ALST
      ...
      if [ $VERBOSE -gt 0 ]; then
        ...
        echo "$0.$LINENO muttley files_0: $tst_files" >&2
      fi
      if [ "$tst_files" != "" ]; then
        for f in $tst_files; do
          if [ $VERBOSE -gt 0 ]; then
            echo "$0.$LINENO try muttley_a file= $f" >&2
          fi
          if [ -e $f ]; then
             OPT_O=
             if [ "$OPTIONS" != "" ]; then
             ...
             if [ $VERBOSE -gt 0 ]; then
                echo $SCR_DIR/resp_2_tsv.sh -w $WORK_DIR/${JOB_ID} -b $BEG_TM -e $END_TM -f $f -s $SUM_ALL $OPT_O $OPT_M >&2
             fi
                echo $SCR_DIR/resp_2_tsv.sh -w $WORK_DIR/${JOB_ID} -b $BEG_TM -e $END_TM -f $f -s $SUM_ALL $OPT_O $OPT_M >&2
          ...
          if [ -e $f.tsv ]; then
             ...
             echo -e "$f.tsv" >> $ALST
             #SHEETS="$SHEETS $f.tsv"
             #echo "$0.$LINENO got latency log $f.tsv" >&2
          fi
        done
      fi
    echo -e "" >> $ALST
  ...
  echo "$0.$LINENO ____got OPT_TM=\"$OPT_TM\"" >&2
  if [ "$END_TM" != "" ]; then
     echo "$0.$LINENO ____got END_TM=\"$END_TM\"" >&2
  ...
  if [ $VERBOSE -gt 0 ]; then
    ...
    echo "$0.$LINENO about to do tsv_2_xls.py" >&2
  fi
 ...
 if [[ "$WPYTHON" != *"python"* ]]; then
  ...
  #if [ $VERBOSE -gt 0 ]; then
    echo "$0.$LINENO $WPYTHON $SCR_DIR/tsv_2_xlsx.py $OPT_SM $OPT_a $OPT_A $OPT_TM -O "$OPTIONS" $OPT_M -f $ALST $SHEETS" >&2
  ...
  #fi
        ...
        PY_PID=$!
        #sleep 1
        #(wait $PY_PID && RC=$? && echo $RC > tsv_2_xls_${JOB_ID}.rc && echo "tsv_2_xls.py $PYPID rc= $RC at $LINENO" >&2) &
        #echo "$0: tsv_2_xlsx.py started with pid= $PY_PID at line= $LINENO" >&2
        #echo $PY_PID >> tsv_2_xlsx.pid
        if [ "$PY_RC" != "0" ]; then
           echo "$0: ================== at line $LINENO: error in tsv_2_xlsx.py. py RC= $PY_RC. See log $FSTDOUT. Bye =================" >&2
           tail -20 $FSTDOUT
           exit 1
        fi
        #if [ ! -e /proc/$PY_PID ]; then
        #   echo "$0: ================== at line $LINENO: error in tsv_2_xlsx.py. py_pid= $PY_PID. See log $FSTDOUT. Bye =================" >&2
        #   tail -20 $FSTDOUT
        #   exit 1
        #else
