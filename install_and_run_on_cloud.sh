...
TAR_GZ=
DRY_RUN=n
RUN_CMDS="ping,shell,command,pcmd,lcl_cmd,ssh_test,opt_reboot,opt_clusto_info,nameserver_test,scp_tar,untar,opt_install_jdk8,opt_install_jdk11,setup,opt_free_up_disk,config,post,run_cpu,run_ncu,run_cmd,run_disk,run_fio,run_both,run_custom,run_multi,run_specjbb,run_specint,run_sysinfo,run_stream,run_geekbench,peek,fetch_untar,get,get_untar,get_recur,report,combine"
...
function print_help() {
      ...
      echo "        If the string contains %HOST% then %HOST% is replaced with the hostname."
      echo "     'ssh_test' ssh's to each box and runs some commands to see if you can ssh to the box. A report of 'worked' 'failed' is produced at the end. Optional cmd"
      echo "     'opt_clusto_info' runs clusto info on each host and outputs results to -o output_file"
...
function tot_compare(i1, v1, i2, v2,    l, r)
{
    ...
    if (m1 < m2)
      ...
      # below is so slow on macbook
      #while IFS= read -r line
      #do
      #    nw_ln=`echo $line | sed 's/#.*//' | sed 's/^[ \t]*//;s/[ \t]*$//' | tr -s ' '`
      #    slen=${#nw_ln}
      #    if [ "$slen" != "0" ]; then
      #      HOSTS="${HOSTS}${SEP}${nw_ln}"
      #      SEP=" "
      #    fi
      #done <"$file"
    fi
...
for i in $HOSTS; do
  ...
  nm=$i
  do_bkgrnd=0
  if [ $RUN_CMDS == "command" -o $RUN_CMDS == "pcmd" ]; then
    do_bkgrnd=1
  fi
  ...
  if [[ $RUN_CMDS == *"opt_clusto_info"* ]]; then
    echo "clusto info $nm"
    if [ "$DRY_RUN" == "n" ]; then
         dyno_log clusto info $nm
         RESP=`clusto info $nm`
  ...
  if [ "$do_untar" == "1" ]; then
    ...
    filenm=$(basename $TAR_GZ)
    ssh_cmd $nm "tar xzf $filenm -C $BMARK_ROOT"  "-l"
    echo ssh $SSH_HOST "$SSH_CMD"
    if [ "$DRY_RUN" == "n" ]; then
         if [ $BKGRND_TASKS_CUR -ge $BKGRND_TASKS_MAX ]; then
           wait
...
#abc
  ...
  if [[ $RUN_CMDS == *"report"* ]]; then
    ...
    echo try clusto info $nm
    if [[ $nm =~ ^[0-9]+ ]]; then
      echo "nm $nm starts with a number so skip clusto info cmd"
      SKU="N/A"
      SKU_MAKE="N/A"
      SKU_MODEL="N/A"
      ...
      clsto_file=$ARCHIVE_DIR/clusto_info.lst
      if [ ! -e $clsto_file ]; then
         clusto info $nm > $clsto_file
      fi
      ...
      echo "from $clsto_file clusto= $CLUSTO"
      else
      CLUSTO=`clusto info $nm`
      fi
      echo did clusto info $nm
