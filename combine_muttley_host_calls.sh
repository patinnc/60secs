...
#echo "$0.$LINENO file list= ${@}" > /dev/stderr
#echo "$0.$LINENO bye" > /dev/stderr
#exit 1
...
awk -v sum_file="$SUM_FILE" -v options="$OPTIONS" -v out_file="$FL_OUT" -v verbose="$VERBOSE" -v rows_max="$ROWS_MAX" -v metric="$METRIC" -v chrt_typ="$CHRT_TYP" -v sheet="$SHEET" -v title="$TITLE" '
   @load "time"
   BEGIN {
...
function sort_a_desc(i1, v1, i2, v2,   lhs, rhs)
{
   ...
   END {
      ...
      tm_diff = ts_end - ts_beg;
      if (tm_diff <= 0.0) { tm_diff = 1.0;}
      if (1==20) {
      for (i=1; i <= hdr_mx-2; i++) {
         j = srt_indx[i];
         nm = hdr_lkup[j];
         printf("nm[%d]= %s\t%.3f\n", i, nm, tots[j]/tm_diff) > "/dev/stderr";
      }
      }
      trow++;
      printf("title\t%s\tsheet\t%s\ttype\tscatter_straight\n", title, sheet) > out_file;
      trow++;
      printf("hdrs\t%d\t%d\t%d\t%d\t%d\n", trow, 2, -1, hdr_mx, 1) > out_file;
      printf("%s\t%s", hdr_lkup[1], hdr_lkup[2]) > out_file;
      for (i=1; i <= hdr_mx-2; i++) {
          j = srt_indx[i];
          nm = hdr_lkup[j];
          printf("\t%s", nm) > out_file;
      }
      trow++;
      printf("\n") > out_file;
      for (r=1; r <= row; r++) {
        printf("%s\t%s", data[r,1], data[r,2]) > out_file;
        for (i=1; i <= hdr_mx-2; i++) {
            j = srt_indx[i];
            printf("\t%f", data[r,j]/fls_mx) > out_file;
        }
        trow++;
        printf("\n") > out_file;
      }
      ...
      if (sum_file != "") {
        for (i=1; i <= hdr_mx-2; i++) {
           ...
           nm = hdr_lkup[j];
           v = tots[j]/fls_mx/row;
           printf("\tmuttley RPS group avg\tRPS grp %s\t%.3f\n", nm, v) >> sum_file;
           #printf("\tmuttley RPS group avg\tRPS grp %s\t%.3f\n", nm, v) > "/dev/stderr"
        }
        close(sum_file);
      }
